...
UPDATE_INTERVAL    = cfg['update_seconds']
...
def run_bot():
    ...
    while True:
        try:
            now = time.time()
            if now - last_param_update_time >= UPDATE_INTERVAL or params is None:
                df = fetch_ohlcv()
            ...
            df = fetch_ohlcv()  # 매 1분마다 현재 가격 정보 확인
            ...
            # ─── 매수 조건 ─────────────────
            if should_buy(ohlcv, params) and rsi < int(params['rsi_threshold']):
